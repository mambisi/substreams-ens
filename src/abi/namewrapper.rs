...
pub mod functions {
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(&[
                ...
                ethabi::Token::Uint(ethabi::Uint::from_big_endian(
                    self.fuse_mask.clone().to_signed_bytes_be().as_slice(),
                )),
            ]);
            let mut encoded = Vec::with_capacity(4 + data.len());
            encoded.extend(Self::METHOD_ID);
    ...
    impl GetFuses {
        const METHOD_ID: [u8; 4] = [74u8, 192u8, 127u8, 65u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            let maybe_data = call.input.get(4..);
            if maybe_data.is_none() {
                return Err("no data to decode".to_string());
    ...
    impl substreams_ethereum::Function for GetFuses {
        const NAME: &'static str = "getFuses";
        fn match_call(call: &substreams_ethereum::pb::eth::v2::Call) -> bool {
            Self::match_call(call)
        }
        fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
    ...
    #[derive(Debug, Clone, PartialEq)]
    pub struct IsTokenOwnerOrApproved {
        pub node: [u8; 32usize],
        pub addr: Vec<u8>,
    }
    impl IsTokenOwnerOrApproved {
        const METHOD_ID: [u8; 4] = [244u8, 71u8, 121u8, 185u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            let maybe_data = call.input.get(4..);
            if maybe_data.is_none() {
                return Err("no data to decode".to_string());
    ...
    impl substreams_ethereum::Function for IsTokenOwnerOrApproved {
        const NAME: &'static str = "isTokenOwnerOrApproved";
        fn match_call(call: &substreams_ethereum::pb::eth::v2::Call) -> bool {
            Self::match_call(call)
        }
        fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
    ...
    impl substreams_ethereum::rpc::RPCDecodable<bool> for IsTokenOwnerOrApproved {
        fn output(data: &[u8]) -> Result<bool, String> {
            Self::output(data)
        }
    }
    ...
    impl RegisterAndWrapEth2ld {
        const METHOD_ID: [u8; 4] = [159u8, 86u8, 218u8, 198u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            let maybe_data = call.input.get(4..);
            if maybe_data.is_none() {
                return Err("no data to decode".to_string());
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(&[
                ...
                ethabi::Token::Uint(ethabi::Uint::from_big_endian(
                    self.fuses.clone().to_signed_bytes_be().as_slice(),
                )),
                ethabi::Token::Uint(ethabi::Uint::from_big_endian(
                    self.expiry.clone().to_signed_bytes_be().as_slice(),
                )),
            ]);
            let mut encoded = Vec::with_capacity(4 + data.len());
            encoded.extend(Self::METHOD_ID);
    ...
    impl Renew {
        const METHOD_ID: [u8; 4] = [104u8, 70u8, 242u8, 247u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            let maybe_data = call.input.get(4..);
            if maybe_data.is_none() {
                return Err("no data to decode".to_string());
    ...
    impl SetFuses {
        const METHOD_ID: [u8; 4] = [194u8, 10u8, 46u8, 176u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            let maybe_data = call.input.get(4..);
            if maybe_data.is_none() {
                return Err("no data to decode".to_string());
    ...
    pub struct SetMetadataService {
        pub new_metadata_service: Vec<u8>,
    }
    ...
    impl SetRecord {
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(&[
                ...
                ethabi::Token::Address(ethabi::Address::from_slice(&self.resolver)),
                ethabi::Token::Uint(ethabi::Uint::from_big_endian(
                    self.ttl.clone().to_signed_bytes_be().as_slice(),
                )),
            ]);
            let mut encoded = Vec::with_capacity(4 + data.len());
            encoded.extend(Self::METHOD_ID);
    ...
    impl Unwrap {
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(&[
                ethabi::Token::FixedBytes(self.parent_node.as_ref().to_vec()),
                ethabi::Token::FixedBytes(self.labelhash.as_ref().to_vec()),
                ethabi::Token::Address(ethabi::Address::from_slice(&self.new_controller)),
            ]);
            let mut encoded = Vec::with_capacity(4 + data.len());
            encoded.extend(Self::METHOD_ID);
            encoded.extend(data);
    ...
    impl UnwrapEth2ld {
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(&[
                ethabi::Token::FixedBytes(self.labelhash.as_ref().to_vec()),
                ethabi::Token::Address(ethabi::Address::from_slice(&self.new_registrant)),
                ethabi::Token::Address(ethabi::Address::from_slice(&self.new_controller)),
            ]);
            let mut encoded = Vec::with_capacity(4 + data.len());
            encoded.extend(Self::METHOD_ID);
    ...
    impl WrapEth2ld {
        const METHOD_ID: [u8; 4] = [249u8, 84u8, 122u8, 158u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            let maybe_data = call.input.get(4..);
            if maybe_data.is_none() {
                return Err("no data to decode".to_string());
    ...
    impl substreams_ethereum::rpc::RPCDecodable<substreams::scalar::BigInt> for WrapEth2ld {
        fn output(data: &[u8]) -> Result<substreams::scalar::BigInt, String> {
            Self::output(data)
        }
    }
...
pub mod events {
    ...
    impl FusesSet {
        const TOPIC_ID: [u8; 32] = [
            147u8, 99u8, 24u8, 226u8, 150u8, 248u8, 36u8, 226u8, 3u8, 176u8, 134u8, 217u8, 123u8,
            177u8, 85u8, 160u8, 32u8, 12u8, 236u8, 72u8, 71u8, 239u8, 234u8, 31u8, 180u8, 185u8,
            183u8, 249u8, 36u8, 21u8, 115u8, 85u8,
        ];
        pub fn match_log(log: &substreams_ethereum::pb::eth::v2::Log) -> bool {
            if log.topics.len() != 2usize {
                return false;
